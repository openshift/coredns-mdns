...
func makeServiceEntry(name string, ip net.IP) *zeroconf.ServiceEntry {
	...
	return &zeroconf.ServiceEntry{
		ServiceRecord: *zeroconf.NewServiceRecord("My Machine", "_http._tcp.", ".local"),
		HostName:      name,
		AddrIPv4:      ipAddrv4,
		AddrIPv6:      ipAddrv6,
	}
...
var (
	ipv4 = makeServiceEntry("mymachine", net.ParseIP("10.1.1.1"))
	ipv6 = makeServiceEntry("mymachine", net.ParseIP("2001::1"))
)
...
func TestAddARecord(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		mutex := sync.RWMutex{}
		m := MDNS{nil, 0, "", &mutex, &hosts, &srvHosts}
		msg := new(dns.Msg)
		reply := new(dns.Msg)
		msg.SetReply(reply)
